...
const Navbar = () => {
  ...
  if (loading || hideNavbarPaths.includes(location.pathname)) return null;
  ...
  const DockItem = ({ to, onClick, icon, label, isActive, colorClass }) => {
    ...
    const content = (
      <>
        <div
          className={`${isActive ? colorClass : "text-gray-500 dark:text-gray-400 group-hover:text-gray-900 dark:group-hover:text-white"}`}
        >
          {icon}
        </div>
        ...
        {isActive && (
          <div
            className={`absolute -bottom-1 w-1 h-1 rounded-full ${colorClass.replace("text-", "bg-")}`}
          ></div>
        )}
      </>
    );
  ...
  return (
    <>
      <nav className="fixed bottom-4 md:bottom-6 left-1/2 -translate-x-1/2 z-50 w-max max-w-[95vw]">
        <div className="flex items-center justify-center gap-1 md:gap-2 px-2 py-2 md:px-4 md:py-3 bg-white/80 dark:bg-slate-900/90 backdrop-blur-2xl border border-white/20 dark:border-slate-700/50 rounded-3xl shadow-[0_10px_30px_-10px_rgba(0,0,0,0.3)] ring-1 ring-black/5 dark:ring-white/10 overflow-visible">
          <DockItem
            to="/"
            icon={<HomeIcon />}
            label="Home"
            ...
            <>
              ...
              >
                <img
                  src={
                    profileData.avatar ||
                    `https://ui-avatars.com/api/?name=${profileData.username || user.email}&background=random`
                  }
                  alt="Profile"
          ...
          <button
            onClick={toggleTheme}
            className="flex-shrink-0 p-2 md:p-3 rounded-full hover:bg-gray-100 dark:hover:bg-slate-800 text-gray-500 dark:text-yellow-400 transition-all hover:-translate-y-1 hover:scale-110 min-w-[40px] flex items-center justify-center"
            title="Toggle Theme"
          >
            {isDarkMode ? "☀️" : "🌙"}
          </button>
